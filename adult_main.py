...
# BACKGROUND
background = pygame.image.load("darkness.jpg")
...
for i in range(num_of_enemies):
    enemy_img.append(pygame.image.load("lala2.png"))
...
# BULLET
bullet_img = pygame.image.load("bra.png")
...
while running:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE:
                if bullet_state == "ready":  # only fire when ready
                    bullet_sound = mixer.Sound("tuturu.wav")
                    bullet_sound.play()
    ...
    for i in range(num_of_enemies):  # to display multiple enemies
        ...
        if collision == True:
            collision_sound = mixer.Sound("moan.wav")
